...
void ADynPropertyObject::DynAddProperty(const AVariant& varValue,
	const char* szName, ASet* pSet, ARange* pRange, int32_t way,
	const char* szPath, bool isVisable, int32_t iState, const char* szDesc)
{
	assert(is_valid());
	m_VarLst.push_back(varValue);
	m_PropLst.push_back(VarPropTemp("", szName, m_VarLst.size() - 1,
		pSet, pRange, way, szPath, isVisable, iState, szDesc));
	m_StateList.push_back(m_PropLst.back().GetState() & AProperty::DEFAULT_VALUE ? 1 : 0);
}

const char* ADynPropertyObject::GetPropDesc(int32_t nIndex)
{
	assert(is_valid());
	return m_PropLst[nIndex].GetDesc();
}

void ADynPropertyObject::SetPropDesc(int32_t nIndex, const char* szDesc)
{
	assert(is_valid());
	m_PropLst.at(nIndex).SetDesc(szDesc);
}

void ADynPropertyObject::SetPropName(int32_t nIndex, const char* name)
{
	assert(is_valid());
...
AProperty* ADynPropertyObject::Properties(const char* name, int32_t* index)
{
	assert(is_valid());
	assert(name);
