...
public:
	...
	AProperty* GetProperty(const wchar_t* name, int32_t* index = nullptr) { return Properties(name, index); }
...
protected:
	...
	virtual AProperty* Properties(const wchar_t* name, int32_t* index = nullptr) { return nullptr; }
...
public:
	...
	void DynAddProperty(const AVariant& varValue,
		const wchar_t* szName, ASet* pSet = nullptr, ARange* pRange = nullptr, int32_t way = WAY_DEFAULT,
		const wchar_t* szPath = nullptr, bool isVisable = true, int32_t iState = 0, const wchar_t* szDesc = 0);

	const wchar_t* GetPropDesc(int32_t nIndex);
	void SetPropDesc(int32_t nIndex, const wchar_t* szDesc);
	void SetPropName(int32_t nIndex, const wchar_t* name);
	void Clear();

protected:
	virtual int32_t PropertiesCount();
	virtual AProperty* Properties(int32_t index);
	virtual AProperty* Properties(const wchar_t* name, int32_t* index = nullptr);
