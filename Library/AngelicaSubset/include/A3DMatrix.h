...
public:
	...
	{
		std::memcpy(m, rkMatrix.m, 9 * sizeof(float));
	}
...
public:
	...
	A3DVECTOR3 GetRow(int32_t i) { return A3DVECTOR3(m[i][0], m[i][1], m[i][2]); }
	A3DVECTOR3 GetCol(int32_t i) { return A3DVECTOR3(m[0][i], m[1][i], m[2][i]); }
...
public:
	...
	A3DMATRIX4(
		float fEntry00, float fEntry01, float fEntry02, float fEntry03,
...
public:
	//	Get row and column
	A3DVECTOR3 GetRow(int32_t i) const { return A3DVECTOR3(m[i][0], m[i][1], m[i][2]); }
	A3DVECTOR3 GetCol(int32_t i) const { return A3DVECTOR3(m[0][i], m[1][i], m[2][i]); }
	//	Set row and column
	void SetRow(int32_t i, const A3DVECTOR3& v) { m[i][0] = v.x; m[i][1] = v.y; m[i][2] = v.z; }
	void SetCol(int32_t i, const A3DVECTOR3& v) { m[0][i] = v.x; m[1][i] = v.y; m[2][i] = v.z; }
